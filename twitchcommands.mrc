on *:TEXT:*!askhelix*:#:{
  ...
  set -u10 %floodhelix. $+ $nick On
  {
    ...
    if (%r == 41) msg $chan Helix: $nick $+ , Select+B+008,135
    if (%r == 42) msg $chan Helix: $nick $+ , Up+A+240,115
    if (%r == 43) msg $chan Helix: $nick $+ , B+R+A
    if (%r == 44) msg $chan Helix: $nick $+ , Start+X+Y
    if (%r == 45) msg $chan Helix: $nick $+ , Up+A+220,115
    if (%r == 46) msg $chan Helix: $nick $+ , Is Slowpoke a Gamer?
    if (%r == 47) msg $chan Helix: $nick $+ , <Removed by Tyrrell>
    if (%r == 48) msg $chan Helix: $nick $+ , Does Mantis like fun?
    if (%r == 49) msg $chan Helix: $nick $+ , After Twitch Plays Pokémon Omega Ruby!
    if (%r == 50) msg $chan Helix: $nick $+ , Ask again when the TPP Park is updated!
    if (%r == 51) msg $chan Helix: $nick $+ , Do I love Lady Helix?
    if (%r == 52) msg $chan Helix: $nick $+ , lotidz!
    if (%r == 53) msg $chan Helix: $nick $+ , Lotid says yes!
    if (%r == 54) msg $chan Helix: $nick $+ , Lotid says no.
    if (%r == 55) msg $chan Helix slaps $nick around a bit with a large trout!
    if (%r == 56) msg $chan Helix: $nick $+ , i dunno
    if (%r == 57) msg $chan Helix: $nick $+ , AW SNAP
...
on *:TEXT:!praiseamber*:#:{
  ...
  {
    if ( $nick == Iwamiger ) { msg $chan ... }
else { msg $chan $nick Salvation Is Balance! $+ } } }
...
; if ($me isop #) && (leonys isop #) { kick # $nick NO PRAISING DOME! }
; elseif ($me isop #) && (leonys !isop #) { msg $chan Nice Try $nick $+ . Both Me and Leonys NEED TO BE OP'd for this to work }
; else { msg $chan Sorry $nick $+ . Both Leonys and I need to be OP'd permanently for this and other new commands to work! Cheers! } }
...
on *:text:!impeach*:#: {
  ...
  {
    if ( $regex($2-,(b.*o|Ο.*t)*d.*o|Ο.*o|Ο.*f.*b.*o|Ο.*t/i) ) { msg $chan ヽ༼ຈل͜ຈ༽ﾉ IMPEACH $upper( $nick ) ヽ༼ຈل͜ຈ༽ﾉ $+ }
    elseif ( $regex($2-,l.*o|Ο.*t.*i.*d/i) ) { msg $chan ヽ༼ຈل͜ຈ༽ﾉ IMPEACH $upper( $nick ) ヽ༼ຈل͜ຈ༽ﾉ $+ }
...
on *:text:blame*:#: {
  ...
  {
    if ( $regex($2-,(b.*o|Ο.*t)*d.*o|Ο.*o|Ο.*f.*b.*o|Ο.*t/i) ) {  msg $chan $me blames $nick $+ }
    elseif ( $regex($2-,l.*o|Ο.*t.*i.*d/i) ) { msg $chan $me blames $nick $+ }
...
on *:TEXT:*doof*:#: { 
  ...
  if ( *kill?*the*r*u*n* iswm $1- ) || ( *kill?*the*u*r*n* iswm $1- ) { msg $chan $nick killed the run }
...
msg $chan Commands for me $nick http://pastebin.com/G93x481E $+ }
