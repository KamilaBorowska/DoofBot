...
else { msg $chan Hey $nick $+ , http://www.twitch.tv/tpp_sideshow $+ } }
...
on $*:TEXT:/(Sun|Flare|Doof)(Sun|Flare|Doof)(Sun|Flare|Doof)/i:#tpprp: { 
...
on *:text:!impeach*:#: {
  ...
  {
    if ( $regex($2-,\b(b.*o|Ο.*t)*d.*o|Ο.*o|Ο.*f.*b.*o|Ο.*t\b/i) ) { msg $chan ヽ༼ຈل͜ຈ༽ﾉ IMPEACH $upper( $nick ) ヽ༼ຈل͜ຈ༽ﾉ $+ }
    elseif ( $regex($2-,\bl.*o|Ο.*t.*i.*d\b/i) ) { msg $chan ヽ༼ຈل͜ຈ༽ﾉ IMPEACH $upper( $nick ) ヽ༼ຈل͜ຈ༽ﾉ $+ }
else { msg $chan ヽ༼ຈل͜ຈ༽ﾉ IMPEACH $upper( $2- ) ヽ༼ຈل͜ຈ༽ﾉ $+ } } }

on $*:TEXT:/\bentei\b/iS:#tpprp: { 
...
msg $chan TPPLeague Metronome Battles here $nick $+ : http://metronome.psim.us/  $+ }
...
on *:text:blame*:#tpprp: {
  ...
  {
    if ( $regex($2-,\b(b.*o|Ο.*t)*d.*o|Ο.*o|Ο.*f.*b.*o|Ο.*t\b/i) ) {  msg $chan $me blames $nick $+ }
    elseif ( $regex($2-,\bl.*o|Ο.*t.*i.*d\b/i) ) { msg $chan $me blames $nick $+ }
    else { msg $chan $nick blames $2- }
  }
}

on *:TEXT:*FlareDoof*:#tpprp: { 
  ;;if ((%floodfd) || ($($+(%,floodfd.,$nick),2))) { return }
  ;;set -u5 %floodfd On
  Vset -u10 %floodfd. $+ $nick On
msg $chan FlareDoof Sucks. SunFlare OTP $+ }

on *:TEXT:*sunflare*:#tpprp: { 
